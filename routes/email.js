...
const sendEmail = async (recipients, subject, text, bookUserId, attachments) => {
  try {
    let mailOptions = {
      from: process.env.EMAIL_USERNAME,
      to: recipients.join(','),
      subject: subject,
      text: `${text}\n\nContribution link: https://www.givebundl.com/contribute/${bookUserId}`,
...
router.post('/start-email-process', async (req, res) => {
    ...
    try {
      ...
      sendEmail(recipients, 'Email 1', 'This is the first email.', savedBook.userID.toString());

      // Schedule the other emails
      setTimeout(() => sendEmail(recipients, 'Email 2', 'This is the second email.', savedBook.userID.toString()), 3 * 24 * 60 * 60 * 1000);
      setTimeout(() => sendEmail(recipients, 'Email 3', 'This is the third email.', savedBook.userID.toString()), 5 * 24 * 60 * 60 * 1000);
      setTimeout(() => sendEmail(recipients, 'Email 4', 'This is the fourth email.', savedBook.userID.toString()), 6 * 24 * 60 * 60 * 1000);
      setTimeout(() => sendEmail(recipients, 'Email 5', 'This is the fifth email.', savedBook.userID.toString()), 7 * 24 * 60 * 60 * 1000);
      ...
      setTimeout(() => {
        ...
        python.on('close', (code) => {
          ...
          sendEmail(recipients, 'Here is your PDF', 'The PDF generated by the Python script is attached.', savedBook.userID.toString(), [
            {
