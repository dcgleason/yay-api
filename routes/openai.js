  ...
  try {
    const gpt4Response = await openai.createChatCompletion({
      ...
      messages: [
        {
          role: 'user',
          content: `Make your reponse 250 characters or less....and return only the final query string....here is the prep process first: give me a comma separated list of gengres associated with the seeed tracks (${seedTracks} and add in the user genre preferences to that list (${userGenrePreference}) -- make these genres work with the spotify api (${availableGenres.data.genres}) . Then this curated list (5 max values for genre) of gengres and seed tracks (as spotify IDs, 5 max spotifyIDs) and make the  query string for a Spotify API request, like this:  https://api.spotify.com/v1/recommendations?seed_artists=4NHQUGzhtTLFvgF5SZesLK&seed_genres=classical%2Ccountry&seed_tracks=0c6xIDDpzE81m2q797ordA. -- return only the final query URL string and include seed tracks (spotify IDs) and seed genre in the query string as both are required.`
        },
      ],
      max_tokens: 200,
    });
